...
function fixFetchCalls(filePath) {
  ...
  // Replace common fetch patterns with apiClient calls
  const replacements = [
    ...
    {
      from: /const response = await fetch\('([^']+)',\s*\{\s*headers:\s*\{\s*'Authorization':\s*`Bearer \$\{token\}`,\s*\}\s*\}\);\s*const data = await response\.json\(\);\s*if \(data\.success\)\s*\{\s*set([^}]+)\s*\(data\);\s*\}/g,
      to: `const data = await apiClient.get('$1', { requireAuth: true, token });\n      if (data.success) {\n        set$2(data);\n      }`
    },
    // POST requests
    {
    ...
    // PUT requests
    {
      from: /const response = await fetch\('([^']+)',\s*\{\s*method:\s*'PUT',\s*headers:\s*\{\s*'Content-Type':\s*'application/json',\s*'Authorization':\s*`Bearer \$\{token\}`\s*\},\s*body:\s*JSON\.stringify\(([^)]+)\)\s*\}\);\s*const data = await response\.json\(\);\s*if \(response\.ok\)\s*\{\s*([^}]+)\s*\}/g,
      to: `const data = await apiClient.put('$1', $2, { requireAuth: true, token });\n      $3`
    },
    // PATCH requests
    {
      from: /const response = await fetch\('([^']+)',\s*\{\s*method:\s*'PATCH',\s*headers:\s*\{\s*'Content-Type':\s*'application/json',\s*'Authorization':\s*`Bearer \$\{token\}`\s*\},\s*body:\s*JSON\.stringify\(([^)]+)\)\s*\}\);\s*const data = await response\.json\(\);\s*if \(response\.ok\)\s*\{\s*([^}]+)\s*\}/g,
      to: `const data = await apiClient.patch('$1', $2, { requireAuth: true, token });\n      $3`
    },
    // DELETE requests
    {
...
function main() {
  console.log('🔧 Fixing ALL fetch calls in admin pages...\n');
