...
async function getHandler(request: NextRequest) {
  try {
    ...
    const typess = await Type.find({ type: 'flights' }).sort({ createdAt: -1 });
...
async function postHandler(request: NextRequest) {
  try {
    ...
    const { title, description, shortDescription, icon, type = 'flights' } = body;
    ...
    console.error('Error creating flights types:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
