...
export const GET = withApiKey(getHandler);: { params: { id: string } }
  ...
  try {
    ...
    const types = await Type.findOne({ _id: params.id, type: 'visas' });
    if (!types) {
    ...
    console.error('Error fetching visas types:', error);
...
export const PUT = withApiKey(putHandler);: { params: { id: string } }
  ...
  try {
    ...
    console.error('Error updating visas types:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export const DELETE = withApiKey(deleteHandler);: { params: { id: string } }
  ...
  try {
    ...
    const types = await Type.findOneAndDelete({ _id: params.id, type: 'visas' });
    if (!types) {
      return NextResponse.json({ error: 'Type not found' }, { status: 404 });
    }
    ...
    console.error('Error deleting visas types:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}