...
const ToursTypesPage: React.FC = () => {
  ...
  const fetchTypes = async () => {
    try {
      const response = await apiClient.get('/api/admin/types?entityType=tour', {
        requireAuth: true,
        token,
      });
      if (response.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingType) {
        await apiClient.put(`/api/admin/tours/types/${editingType._id}`, formData, {
          requireAuth: true,
          token,
        });
      } else {
        await apiClient.post('/api/admin/types', formData, {
          requireAuth: true,
          token,
        });
      }
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this type?')) {
      try {
        const response = await fetch(`/api/admin/tours/types/${id}`, {
          method: 'DELETE',
          headers: {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage tour types and classifications</p>
        </div>
        <button
