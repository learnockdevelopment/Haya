  ...
  const fetchData = async () => {
    try {
      const [categoriesRes, typesRes, tagsRes, regionsRes] = await Promise.all([
        apiClient.get('/api/admin/categories?entityType=hotel', { requireAuth: true, token }), {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        apiClient.get('/api/admin/types?entityType=hotel', { requireAuth: true, token }), {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        apiClient.get('/api/admin/tags?entityType=hotel', { requireAuth: true, token }), {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Add a new hotel to the system</p>
          </div>
        </div>
      </div>
