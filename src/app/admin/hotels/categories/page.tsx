...
const HotelCategorysPage: React.FC = () => {
  const { token } = useAuth();
  const { t } = useLanguage();
  ...
  const fetchCategorys = async () => {
    try {
      const response = await apiClient.get('/api/admin/categories?entityType=hotel', { requireAuth: true, token }), {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingCategory 
        ? `/api/admin/hotels/categories/${editingCategory._id}`
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this categories?')) {
      try {
        const response = await fetch(`/api/admin/hotels/categories/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Hotel Categorys</h1>
          <p className="text-gray-600">Manage hotels categories and classifications</p>
        </div>
        <button
...
export default HotelCategorysPage;