...
const VisaTypesPage: React.FC = () => {
  ...
  const fetchTypes = async () => {
    try {
      const response = await apiClient.get('/api/admin/types?entityType=visa', { requireAuth: true, token }), {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingType 
        ? `/api/admin/visas/types/${editingType._id}`
        : '/api/admin/types';
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this types?')) {
      try {
        const response = await fetch(`/api/admin/visas/types/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage visas types and classifications</p>
        </div>
        <button
