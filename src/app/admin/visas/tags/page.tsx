...
const VisaTagsPage: React.FC = () => {
  ...
  const fetchTags = async () => {
    try {
      const response = await apiClient.get('/api/admin/tags?entityType=visa', { requireAuth: true, token }), {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingTag 
        ? `/api/admin/visas/tags/${editingTag._id}`
        : '/api/admin/tags';
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this tags?')) {
      try {
        const response = await fetch(`/api/admin/visas/tags/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Visa Tags</h1>
          <p className="text-gray-600">Manage visas tags and classifications</p>
        </div>
        <button
